/**
 * Contador de tentativas de login da área do cliente: falhas seguidas bloqueiam
 * a chave por 5 minutos.
 *
 * Não confundir com `rate-limit.ts`, que é outra coisa: lá é o throttle de
 * volume por IP (15 envios por minuto), aplicado pelo middleware a *todas* as
 * server functions, contando toda requisição. Aqui só contam as tentativas que
 * **falharam**, e por credencial — é a trava contra adivinhar senha, não contra
 * volume. As duas valem ao mesmo tempo no /cliente.
 *
...
export type VereditoTentativas = { blocked: false } | { blocked: true; retryAfterSeconds: number };

const veredito = (retryAfterMs: number): VereditoTentativas =>
  retryAfterMs > 0
 ...
 */
export function checkTentativas(keys: LimitKey[]): VereditoTentativas {
  const now = Date.now();
  let retryAfterMs = 0;
...
export function registrarFalha(keys: LimitKey[]): VereditoTentativas {
  const now = Date.now();
  sweep(now);
  let retryAfterMs = 0;
...
export function limparTentativas(keys: LimitKey[]) {
  for (const { key } of keys) entries.delete(key);
}

/** "5 minutos" / "40 segundos", para a mensagem que o cliente lê. */
export function mensagemDeBloqueio(retryAfterSeconds: number) {
